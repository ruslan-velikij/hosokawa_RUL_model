...
MAKE_ROLLING_FEATURES = True

ROLLING_WINDOWS_ROWS = {
    "1h": 60,
    "6h": 360,
}
...
ROLLING_AGGREGATIONS = ("mean", "std", "range")
...
def add_rolling_features(df: pd.DataFrame, signal_cols: list[str]) -> pd.DataFrame:
    ...
    for col in signal_cols:
        log(f"  rolling для признака: {col}")
        for window_label, window_size in ROLLING_WINDOWS_ROWS.items():
            rolling_obj = grouped[col].rolling(
                window=window_size,
                min_periods=ROLLING_MIN_PERIODS,
            )

            prefix = f"{col}_roll_{window_label}"

            if "mean" in ROLLING_AGGREGATIONS:
                new_features[f"{prefix}_mean"] = (
                    rolling_obj.mean().reset_index(level=0, drop=True).astype("float32")
                )

            if "std" in ROLLING_AGGREGATIONS:
                new_features[f"{prefix}_std"] = (
                    rolling_obj.std().reset_index(level=0, drop=True).astype("float32")
                )

            if "min" in ROLLING_AGGREGATIONS or "range" in ROLLING_AGGREGATIONS:
                roll_min = rolling_obj.min().reset_index(level=0, drop=True).astype("float32")
            else:
                roll_min = None

            if "max" in ROLLING_AGGREGATIONS or "range" in ROLLING_AGGREGATIONS:
                roll_max = rolling_obj.max().reset_index(level=0, drop=True).astype("float32")
            else:
                roll_max = None

            if "min" in ROLLING_AGGREGATIONS and roll_min is not None:
                new_features[f"{prefix}_min"] = roll_min

            if "max" in ROLLING_AGGREGATIONS and roll_max is not None:
                new_features[f"{prefix}_max"] = roll_max

            if "range" in ROLLING_AGGREGATIONS and roll_min is not None and roll_max is not None:
                new_features[f"{prefix}_range"] = (roll_max - roll_min).astype("float32")

    if new_features:
        rolling_df = pd.DataFrame(new_features, index=df.index)
        df = pd.concat([df, rolling_df], axis=1, copy=False)
        log(f"добавлено rolling-признаков: {len(new_features)}")
...
def fill_missing_values(df: pd.DataFrame, node_name: str) -> tuple[pd.DataFrame, dict[str, float]]:
    ...
    for col in fill_cols:
        if df[col].isna().any():
            df[col] = grouped[col].ffill(limit=FFILL_LIMIT)

        median = df.loc[train_mask, col].median()
        if pd.isna(median):
            median = 0.0
        medians[str(col)] = float(median)
...
def prepare_one_dataset(node_name: str, input_path: Path, output_cfg: dict[str, Path]) -> None:
    ...
    summary["rolling_min_periods"] = ROLLING_MIN_PERIODS if MAKE_ROLLING_FEATURES else None
    summary["rolling_aggregations"] = list(ROLLING_AGGREGATIONS) if MAKE_ROLLING_FEATURES else []

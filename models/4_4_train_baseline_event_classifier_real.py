...
OUT_DIR = PROCESSED_DIR / "baseline_event_classifier_real_signals_only"
...
def get_feature_columns(df: pd.DataFrame, target: str) -> list[str]:
    ...
    for col in df.columns:
        if col in excluded:
            continue
        if col in EXCLUDE_SERVICE_FEATURES:
            continue
        if col.endswith(EXCLUDE_FEATURE_SUFFIXES):
            continue
...
def process_node(node: str, paths: dict) -> dict:
    ...
    result = {
        ...
        "settings": {
            ...
            "exclude_service_features": sorted(EXCLUDE_SERVICE_FEATURES),
            "exclude_feature_suffixes": list(EXCLUDE_FEATURE_SUFFIXES),

...
def find_suspicious_columns(columns: list[str]) -> list[str]:
    ...
    for col in columns:
        ...
        if "\\" in col:
            suspicious.append(f"{col} -> техническое имя из SCADA/источника, надо уточнять")
...
def analyze_parquet(file_path: Path) -> None:
    ...
    # 3. Regim
    print("[3/4] Считаются режимы Regim...")
    ...
    # 4. Группы и спорные столбцы
    print("[4/4] Группируются столбцы по смыслу...")
    ...
    print("\n3) ПРОПУСКИ ПО СТОЛБЦАМ")
    print(format_series(na_counts))

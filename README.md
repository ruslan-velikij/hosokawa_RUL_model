...
4) Запустить эксперименты модели — при необходимости
   3 — основной результат для мельницы
   7 — дополнительный результат для компактора с SSD
5) Запустить дополнительный RUL-анализ — при необходимости
   5_1 — интервалы между отказами
   5_2 — регрессия календарного времени до отказа
   5_3 — анализ ложных срабатываний
   5_4 — рабочие часы до отказа
```
...
│   ├── 5_2_train_rul_regressor.py
│   ├── 5_3_analyze_false_alarms.py
│   └── 5_4_calculate_operating_hours_to_failure.py
├── processed/
├── toir_hosokawa/
├── results/
└── requirements.txt
...
* `hosokawa_RUL_models.py` — управляющий консольный скрипт для запуска основных этапов, экспериментов и дополнительного RUL-анализа;
...
4. Запустить или пропустить эксперименты модели.
5. Запустить или пропустить дополнительный RUL-анализ.

Запуск экспериментов теперь является опциональным. Если на вопрос о запуске экспериментов ответить `y`, скрипт покажет список сценариев 1–7. После завершения выбранных экспериментов будет предложено запустить дополнительный RUL-анализ. Если на вопрос о запуске экспериментов ответить `n`, управляющий скрипт сразу перейдёт к вопросу о запуске RUL-анализа.
...
## Дополнительный RUL-анализ

После обучения классификационной модели можно запустить дополнительный анализ в направлении остаточного ресурса. Этот блок добавлен для проверки не только факта приближения технического события, но и временных характеристик отказов.

Управляющий скрипт запускает RUL-анализ опционально. Если выбрать `y`, последовательно выполняются четыре сценария:

```bash
python models/5_1_analyze_failure_intervals.py
python models/5_2_train_rul_regressor.py
python models/5_3_analyze_false_alarms.py
python models/5_4_calculate_operating_hours_to_failure.py
```

Лог команд сохраняется в файле:

```text
results/rul_analysis_run_commands.txt
```

Перед запуском RUL-анализа должны быть подготовлены файлы в `processed/`. Обычно для этого сначала запускается эксперимент 3, так как он формирует рабочую постановку: события класса «Неисправность/отказ» и реальные технологические признаки.

### Сценарий 5_1. Анализ интервалов между отказами
...
* читает события из `toir_hosokawa/jtiny_hosokawa_events_by_node.xlsx`;
* берёт листы `mill` и `compactor`;
* оставляет события класса `Неисправность/отказ`;
* считает интервалы между соседними отказами по узлам и установкам `N_Hosokawa`.

Результаты сохраняются в:

```text
results/failure_intervals/
```

Основные выходные файлы:

```text
failure_intervals_by_node.csv
failure_intervals_summary.csv
failure_intervals.xlsx
```

Этот сценарий нужен, чтобы понять, как часто в журналах повторяются отказные события: через часы, дни, недели или месяцы.

### Сценарий 5_2. Регрессия календарного времени до отказа
...
* использует цель `time_to_next_event_hours`;
* обучает регрессионную модель `HistGradientBoostingRegressor`;
* оценивает прогноз времени до ближайшего отказа в пределах горизонтов 72, 168 и 336 часов.

По умолчанию рассматриваются строки:
...
```text
results/rul_regression_calendar/
```
...
rul_regression_metrics.json
rul_regression_summary.txt
*_predictions_sample.csv
```
...
| `MedianAE` | медианная абсолютная ошибка в часах |
| `RMSE` | корень из средней квадратичной ошибки, сильнее штрафует крупные промахи |
| `R²` | коэффициент детерминации, показывает объясняющую способность модели |

Этот сценарий нужен для перехода от классификации события к задаче оценки времени, оставшегося до отказа.

### Сценарий 5_3. Анализ ложных срабатываний
...
* ищет файлы `*_threshold_metrics*.csv` в `results/`;
* рассчитывает дополнительные показатели ошибок классификации;
* показывает, сколько предупреждений модели являются ложными и сколько событий модель пропускает.
...
```text
false_alarm_analysis_by_threshold.csv
false_alarm_summary_by_experiment.csv
false_alarm_analysis.xlsx
...
| Показатель | Смысл |
| --- | --- |
| `TP` | модель предупредила, и событие действительно было |
| `FP` | модель предупредила, но события не было |
| `TN` | модель не предупредила, и события не было |
| `FN` | модель не предупредила, но событие было |
| `specificity` | доля правильно распознанных обычных состояний |
| `false_positive_rate` | доля ложных тревог среди обычных состояний |
| `false_discovery_rate` | доля ложных тревог среди всех предупреждений модели |
| `negative_predictive_value` | насколько можно доверять прогнозу «события не будет» |

Этот сценарий нужен для практической оценки модели: оператору важно знать не только полноту обнаружения отказов, но и цену ложных предупреждений.

### Сценарий 5_4. Расчёт рабочих часов до отказа
...
* читает `processed/compactor_dataset_labeled.parquet` и `processed/mill_dataset_labeled.parquet`;
* определяет, работает ли оборудование, по скоростям и токам;
* считает не только календарное время до события, но и фактические рабочие часы до ближайшего отказа.

Результаты сохраняются в:

```text
results/operating_hours_to_failure/
```

Основные выходные файлы:

```text
operating_hours_summary_by_node.csv
operating_hours_summary_by_unit.csv
operating_hours_to_failure_sample.csv
operating_hours_to_failure.xlsx
operating_hours_params.json
operating_hours_summary.txt
```
...
| Поле | Смысл |
| --- | --- |
| `is_running` | признак работы оборудования на строке временного ряда |
| `running_interval_hours` | рабочая длительность интервала до следующей строки |
| `operating_hours_to_next_failure` | сумма рабочих часов от текущей строки до ближайшего события |
| `operating_to_calendar_ratio` | отношение рабочих часов до события к календарным часам до события |

Этот сценарий важен для RUL-постановки, потому что календарные часы не всегда отражают фактическую наработку оборудования. Если установка значительную часть времени стоит, то до отказа может оставаться много календарных часов, но мало рабочих часов.
...
Дополнительные RUL-результаты сохраняются отдельно:

```text
results/failure_intervals/              # интервалы между отказами
results/rul_regression_calendar/        # регрессия времени до отказа
results/error_analysis/                 # ложные срабатывания
results/operating_hours_to_failure/     # рабочие часы до отказа
```
...
Дополнительно реализован блок RUL-анализа. Он позволяет оценить интервалы между отказами, построить регрессионную модель календарного времени до отказа, разобрать ложные срабатывания классификатора и рассчитать фактические рабочие часы до события. За счёт этого проект закрывает не только задачу бинарного предупреждения о техническом событии, но и часть задачи оценки остаточного ресурса оборудования.
